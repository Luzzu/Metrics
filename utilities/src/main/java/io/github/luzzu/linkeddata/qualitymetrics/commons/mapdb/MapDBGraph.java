...
public class MapDBGraph {
	...
	private DB mapDB = MapDbFactory.getMapDBAsyncTempFile(MapDBGraph.class.getName());
	  
	private HTreeMap<String, Set<String>> adjListInEdges = MapDbFactory.createHashMap(mapDB, "graph-inedges-map");
	private HTreeMap<String, Set<String>> adjListOutEdges = MapDbFactory.createHashMap(mapDB,"graph-outedges-map");
	private HTreeMap<String, Set<String>> sameAsNodes = MapDbFactory.createHashMap(mapDB,"sameAsNodes");
	
	//Adjacency Matrix
//	private HTreeMap<Integer, HTreeMap<Integer,Byte>> A = this.mapDB.createHashMap("graph-adjacency-matrix").make();
	private HTreeMap<String, Integer> A_index = MapDbFactory.createHashMap(mapDB,"graph-adjacency-matrix-index"); // holds the ith index of the node in the adjacency matrix
	private HTreeMap<Integer, String> A_reverse_index = MapDbFactory.createHashMap(mapDB,"graph-adjacency-matrix-reverse-index"); // holds the ith index of the node in the adjacency matrix

	
	//we need something to store the edge name (i.e property name)

	private Set<String> hashSet = MapDbFactory.createHashSet(mapDB, "nodes-hashSet");
