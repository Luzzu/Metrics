 ...
 */
public class ExtensionalConciseness extends AbstractQualityMetric<Double> {
	 ...
	 */
	private DB mapDB = MapDbFactory.getMapDBAsyncTempFile(ExtensionalConciseness.class.getName());
	 ...
	 */
	private HTreeMap<String, ComparableSubject> pMapSubjects = MapDbFactory.createHashMap(mapDB, "extensional-conciseness-map");
	...
	public Double metricValue() {
		...
		EnclosingClassVars vars = new EnclosingClassVars();
		
		int mapSize = pMapSubjects.size();
		logger.debug("Starting metric value computation, with " + mapSize + "instances...");
		
		
		pMapSubjects.getValues().forEach(curSubject -> {
			vars.isCurSubjectUnique = true;
			...
			if(vars.isCurSubjectUnique) {
				lstUniqueSubjects.add(curSubject);
			}
			
			vars.countCompared++;
			if((vars.countCompared % 50000) == 0) {
				logger.debug("Estimated value with " + vars.countCompared + " compared instances: " +  (((double)lstUniqueSubjects.size()) / ((double)mapSize)));
			}
		});
...
//		MapDbFactory.invokeClose(ExtensionalConciseness.class.getName(), mapDB);
	...
	private class EnclosingClassVars {
		protected boolean isCurSubjectUnique = false;
		protected long countCompared = 0l;
	}
